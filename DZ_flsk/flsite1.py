...
import sqlite3
import os
from DZ_flsk.FDataBase import FDataBase

DATABASE = 'flsk2.db'
DEBUG = True
SECRET_KEY = 'fdhgfbyblnoiuno123unoiubiyvvyucy'
app = Flask(__name__)
app.config.from_object(__name__)
app.config.update(dict(DATABASE=os.path.join(app.root_path, "flsk2.db")))
...
def index():
    db = get_db()
    dbase = FDataBase(db)
    return render_template("index.html", title='Pricing', menu=dbase.get_menu(), course=dbase.get_course_anonce())
...
def add_course():
    ...
    if request.method == 'POST':
        if len(request.form['name']) > 4 and len(request.form['course']) > 10:
            res = dbase.add_course(request.form['name'], request.form['course'], request.form['price'], request.form['url'])
            if not res:
                flash('Ошибка добавление курса', category='error')
            else:
...
def show_course(alias):
    db = get_db()
    dbase = FDataBase(db)
    title, course, price  = dbase.get_course(alias)
    if not title:
        abort(404)
    return render_template("course.html", menu=dbase.get_menu(), title=title, course=course, price=price)
